...
them to CSV so we can for instance share them with our collaborators.
...
> ## Instructor note
>
> Demonstrate that the function works (it returns a data.frame with all
> countries combined).
...
> ### Instructor notes
>
> Demonstrate how to write this function. Start by just typing the write.csv
> line and expends from there. We use `row.names=FALSE` because we don't want
> them in the output, `verbose` just write something at the terminal which will
> be useful when we are building the full manuscript to keep track of
> progress. You may want to leave out the `...` depending on the participants'
> level of comfort with R code.
