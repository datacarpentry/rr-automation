...
>  - `tidyverse`
>  - `testthat`
>  - `rmarkdown` and `knitr` (in RStudio pushing the knit HTML button the first time will prompt to have these packages installed). **Note** -  you may need to tell `RStudio` to use `knitr`, do this by: `Preferences` > `Sweave` find "weave Rnw files using:" and select `knitr`. 
